/**
 * Email Verification Screen - First step in registration flow
 * @format
 */
  ...
  const handleSendCode = async () => {
    ...
    try {
      ...
      if (response.success) {
        showToast.success(
          'Code Sent',
          `Verification code has been sent to ${email.trim()}`
        );
        // Navigate to code verification screen
        navigation.navigate('CodeVerification', { email: email.trim() });
      } else {
        console.warn('⚠️ Email verification failed:', response.message);
      ...
      if (err.response) {
        ...
        errorMessage = 'Network error: Unable to reach server. Please check your internet connection and API URL.';
        console.error('🌐 Network error details:', {
          baseURL: err.config?.baseURL,
          url: err.config?.url,
          timeout: err.code === 'ECONNABORTED' ? 'Request timeout' : 'Connection failed',
        });
      } else {
        ...
        <View style={styles.formSection}>
          <View style={styles.inputContainer}>
            ...
            <TextInput
              ...
              onChangeText={(text) => {
                setEmail(text);
                // Reset registered state when user changes email
          ...
          {isEmailRegistered && (
            <View style={styles.registeredContainer}>
              <Text style={styles.registeredTitle}>Email Already Registered</Text>
              <Text style={styles.registeredMessage}>
